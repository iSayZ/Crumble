...
const readFriendRequest = async (req, res, next) => {
  try {
...
const add = async (req, res, next) => {
  ...
  try {
    ...
    // Pass any errors to the error-handling middleware
    if (err === 1062) {
      next(res.status(500).json({ error: 'Cette adresse e-mail est déjà utilisée.'}));
    } else {
      next(res.status(500).json({ error: 'Erreur lors de la création du compte.' }));
    }
  }
};
