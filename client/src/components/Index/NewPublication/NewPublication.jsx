...
function NewPublication () {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await myAxios.post("/api/publications", {
    ...
    return (
        <div className="new-publication-page">
            <form className="new-publication-container" onSubmit={handleSubmit}>
                <button type="button" onClick={handleCloseNewPublication} className="no-btn close pointer">
                    <img src="/assets/images/icons/close-b.svg" alt="Bouton de fermeture de la fênetre de création de publication." />
                </button>
                <div className="new-publication-header">
                    ...
                    id="fileInput"
                    />
                    <button type="button" className="no-btn add-picture pointer" onClick={() => fileInput.current.click()}>
                        <AddCircleOutlineIcon />
                        <p>{selectedFile ? selectedFile.name : "Ajouter une image (optionnel)"}</p>
                    </button>
                    <p className="error">{error}</p>
                </div>
                <ReactQuill placeholder="Bonjour à tous..." ref={quillRef} className="editor" value={publication} onChange={setPublication} />
                <div className="new-publication-footer">
                    <button type="submit" className="btn">Créer la publication</button>
                </div>
            </form>
        </div>
    )
